...
print(f"Nuking ALL keepNext black dots from ALL paragraphs across {FOXCONNECT_ROOT}...")
...
for root, dirs, files in os.walk(FOXCONNECT_ROOT):
    ...
    # Extra safety check: strip keep_with_next from EVERY paragraph in the document
    ...
    try:
        ...
        print(f"  [ZERO KEEP_NEXT BLACK DOTS - CLEAN] {new_docx_filename}")
        generated_notes += 1
    except Exception as e:
...
print(f"\n==========================================")
print(f"Finished nuking keepNext from all 29 property notes! Total folders: {processed_folders}")
