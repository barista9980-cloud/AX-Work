...
def format_cell(cell, text, bold=False, font_size=10.0, align=WD_ALIGN_PARAGRAPH.CENTER):
    ...
    # Ensure empty cells have 10pt 맑은 고딕 default run
    if not p.runs:
        p.add_run()
    for r in p.runs:
        r.font.name = "맑은 고딕"
...
for s_dir in sub_dirs:
    ...
    for u_dir in unit_dirs:
        ...
        # Purge ALL empty paragraphs from document body XML
        for p in list(doc.paragraphs):
            if not p.text.strip():
                p._element.getparent().remove(p._element)

        # P0 Title: 주제목 13pt Bold 맑은 고딕
        doc.paragraphs[0].text = f"[{b_name}] {u_name} 부동산 계약 관리 노트"
        doc.paragraphs[0].paragraph_format.keep_with_next = False
        ...
        # Section 1 Heading: P2 -> 소제목 11pt Bold 맑은 고딕 ("1. 주 계약 정보")
        ...
        # Table 0: 주 계약 정보 (Cell font 10pt)
        t0 = doc.tables[0]
        ...
        # Section 2 Heading: P3 -> 소제목 11pt Bold 맑은 고딕 ("2. 수록 계약서 문서 목록 (총 N건)")
        ...
        # Section 3 Heading: P4 -> 소제목 11pt Bold 맑은 고딕 ("3. 임대인 및 납부 계좌 정보")
        ...
        # Table 2: 임대인 및 계좌 정보 (Cell font 10pt)
        t2 = doc.tables[2]
        ...
        # Section 4 Heading: P5 -> 소제목 11pt Bold 맑은 고딕 ("4. 계약 변동 이력 및 특이사항")
        ...
        try:
            ...
            print(f"    [CREATED PERFECT NOTE 10pt, Title 13pt, Sec 11pt, Dot Restored] {new_docx_filename}")
            processed_count += 1
        except Exception as e:
            print(f"    [ERROR SAVING] {new_docx_filename}: {e}")

print(f"\n==========================================")
print(f"Finished processing 01_가산 to 04_광명! Total notes generated: {processed_count}")
print(f"==========================================")
