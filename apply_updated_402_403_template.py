...
output_docx_path_alt = r"C:\Users\User\OneDrive\바탕 화면\부동산_계약관리노트_강남_도곡로1길23_신규양식.docx"
...
# 1. Update Title (P0)
doc.paragraphs[0].text = "[강남_도곡로1길23] 전층 부동산 계약 관리 노트"
for r in doc.paragraphs[0].runs:
    r.font.name = "맑은 고딕"
    r.font.size = Pt(15.0)
    r.bold = True

# 2. Update Table 0 (주 계약 정보)
t0 = doc.tables[0]
t0_data = [
...
# 3. Update Heading P3 (2. 수록 계약서 문서 목록)
doc.paragraphs[3].text = "2. 수록 계약서 문서 목록 (총 8건)"
for r in doc.paragraphs[3].runs:
    r.font.name = "맑은 고딕"
    r.bold = True

# 4. Update Table 1 (5컬럼: 순서 | 문서명 | 계약 종류 | 계약 당사자 (임대인 - 임차인) | 계약일)
t1 = doc.tables[1]

# Clear existing data rows, keeping header row 0
while len(t1.rows) > 1:
    t1._tbl.remove(t1.rows[-1]._tr)

docs_data = [
    ("01", "강남_도곡로1길23_전층_01_최초임대차_[박재윤-㈜폭스에듀]_(241107)", "최초임대차", "박재윤 → ㈜폭스에듀", "2024-11-07"),
    ("02", "강남_도곡로1길23_전층_02_변경계약_[유한회사 청송(박재윤)-㈜폭스에듀]_(250901)", "변경계약", "유한회사 청송(박재윤) → ㈜폭스에듀", "2025-09-01"),
    ("03", "강남_도곡로1길23_1층_01_전대차_[㈜폭스에듀-㈜에스앤에이치트레이딩]_(241101)", "전대차", "㈜폭스에듀 → ㈜에스앤에이치트레이딩", "2024-11-01"),
    ("04", "강남_도곡로1길23_1층_02_전대차_[㈜폭스에듀-한국경찰과학전략센터]_(250821)", "전대차", "㈜폭스에듀 → 한국경찰과학전략센터", "2025-08-21"),
    ("05", "강남_도곡로1길23_1층_03_전대차_[㈜폭스에듀-㈜월드유니코어]_(250821)", "전대차", "㈜폭스에듀 → ㈜월드유니코어", "2025-08-21"),
    ("06", "강남_도곡로1길23_2층_01_전대차_[㈜폭스에듀-㈜실리콘아츠]_(241101)", "전대차", "㈜폭스에듀 → ㈜실리콘아츠", "2024-11-01"),
    ("07", "강남_도곡로1길23_2층_02_전대차_[㈜폭스에듀-㈜하이퍼비주얼에이아이]_(250101)", "전대차", "㈜폭스에듀 → ㈜하이퍼비주얼에이아이", "2025-01-01"),
    ("08", "강남_도곡로1길23_3층_01_전대차_[㈜폭스에듀-㈜트라이디스]_(250124)", "전대차", "㈜폭스에듀 → ㈜트라이디스", "2025-01-24")
]
...
# 5. Update Table 2 (임대인 및 납부 계좌 정보)
t2 = doc.tables[2]
t2_data = [
...
# 6. Update Table 3 (계약 변동 이력 및 특이사항)
t3 = doc.tables[3]
history_items = [
...
try:
    ...
    saved_file = output_docx_path
except Exception as e:
    doc.save(output_docx_path_alt)
    saved_file = output_docx_path_alt

print("Saved updated docx to:", saved_file)
