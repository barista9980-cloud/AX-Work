...
print(f"Applying Smart Clean Table Page Break Rules across {FOXCONNECT_ROOT}...")
...
for root, dirs, files in os.walk(FOXCONNECT_ROOT):
    ...
    # SMART PAGE BREAK RULE:
    # If the document has 3+ contracts (Table 1 has >=4 rows) or if content overflows 1 page,
    # insert a clean Page Break before Section 3 so that Sections 1-2 stay on Page 1 and Sections 3-4 start cleanly at Top of Page 2!
    if len(contract_items) >= 3:
        doc.add_page_break()
        print("  [SMART PAGE BREAK INSERTED BEFORE SECTION 3 FOR READABILITY]")
    ...
    try:
        ...
        print(f"  [SMART CLEAN PAGE BREAK RULE APPLIED] {new_docx_filename}")
        generated_notes += 1
    except Exception as e:
...
print(f"\n==========================================")
print(f"Finished applying smart page break rules across all 29 property notes! Total folders: {processed_folders}")
