...
TEMPLATE_BASE = r"C:\Users\User\OneDrive\바탕 화면\부동산_계약관리노트_402_403호.docx"

# Find 01_임대차 parent folder
target_parent = None
...
print(f"Target Parent Path: {target_parent}")
...
def format_cell(cell, text, bold=False, font_size=11.0, align=WD_ALIGN_PARAGRAPH.CENTER):
    cell.text = text
    p = cell.paragraphs[0]
    p.alignment = align
    ...
    # Ensure there is at least one run formatted in 맑은 고딕 11pt even for empty text
    if not p.runs:
        p.add_run()
    for r in p.runs:
        r.font.name = "맑은 고딕"
...
for s_dir in sub_dirs:
    ...
    print(f"\n==========================================")
    print(f"Processing Target Folder Group: {s_dir}")
    print(f"==========================================")

    unit_dirs = os.listdir(folder_path)
    unit_dirs.sort()
    ...
    for u_dir in unit_dirs:
        ...
        print(f"\n -> Processing Unit Folder: {u_dir}")

        files_in_unit = os.listdir(u_path)

        # 1. Delete existing contract note files
        existing_notes = [f for f in files_in_unit if f.lower().endswith(".docx") and ("계약관리노트" in f or "계약_노트" in f or "노트" in f)]
        for old_f in existing_notes:
            old_p = os.path.join(u_path, old_f)
            try:
                os.remove(old_p)
        ...
        # 2. Find PDF contract files
        pdf_files = [f for f in files_in_unit if f.lower().endswith(".pdf")]
        if not pdf_files:
        ...
        # Load fresh Document
        doc = docx.Document(TEMPLATE_BASE)

        # Set Normal Style default font to 맑은 고딕 11pt
        ...
        # Purge ALL empty paragraphs from document body XML (eliminates 2 extra enters between sec2 and sec3)
        for p in list(doc.paragraphs):
            if not p.text.strip():
                p._element.getparent().remove(p._element)

        # P0 Title: 15pt Bold 맑은 고딕
        ...
        # Section 1 Heading: P2 (13pt Bold 맑은 고딕)
        sec1_p = doc.paragraphs[2]
        sec1_p.text = "1. 주 계약 정보"
        sec1_p.paragraph_format.keep_with_next = True
        for r in sec1_p.runs:
            r.font.name = "맑은 고딕"
            r.font.size = Pt(13.0)
            r.bold = True

        # Table 0: 주 계약 정보 (Cell font 11pt)
        t0 = doc.tables[0]
        ...
        for r_i, r_row in enumerate(t0.rows):
            ...
            for c_i, (val, is_h) in enumerate(row_vals):
                ...
                format_cell(cell, val, bold=is_h, font_size=11.0, align=align)
                if is_h:
                    set_cell_background(cell, "F2F4F7")

        # Section 2 Heading: P3 (13pt Bold 맑은 고딕)
        sec2_p = doc.paragraphs[3]
        sec2_p.text = f"2. 수록 계약서 문서 목록 (총 {len(contract_items)}건)"
        sec2_p.paragraph_format.keep_with_next = True
        ...
        for c_i, h in enumerate(t1_headers):
            ...
            format_cell(cell, h, bold=True, font_size=11.0, align=WD_ALIGN_PARAGRAPH.CENTER)
        ...
        for idx, c_it in enumerate(contract_items, 1):
            ...
            format_cell(r_cells[0], seq_str, bold=False, font_size=11.0, align=WD_ALIGN_PARAGRAPH.CENTER)
            format_cell(r_cells[1], f_name, bold=False, font_size=11.0, align=WD_ALIGN_PARAGRAPH.LEFT)
            format_cell(r_cells[2], c_type, bold=False, font_size=11.0, align=WD_ALIGN_PARAGRAPH.CENTER)
            format_cell(r_cells[3], parties_str, bold=False, font_size=11.0, align=WD_ALIGN_PARAGRAPH.CENTER)
            format_cell(r_cells[4], c_date, bold=False, font_size=11.0, align=WD_ALIGN_PARAGRAPH.CENTER)

        # Section 3 Heading: P4 (13pt Bold 맑은 고딕) - Directly follows Table 1 without any blank paragraphs!
        sec3_p = doc.paragraphs[4]
        sec3_p.text = "3. 임대인 및 납부 계좌 정보"
        sec3_p.paragraph_format.keep_with_next = True
        for r in sec3_p.runs:
            r.font.name = "맑은 고딕"
            r.font.size = Pt(13.0)
            r.bold = True

        # Table 2: 임대인 및 계좌 정보 (Cell font 11pt)
        t2 = doc.tables[2]
        ...
        # Section 4 Heading: P5 (13pt Bold 맑은 고딕)
        sec4_p = doc.paragraphs[5]
        sec4_p.text = "4. 계약 변동 이력 및 특이사항"
        sec4_p.paragraph_format.keep_with_next = True
        for r in sec4_p.runs:
            r.font.name = "맑은 고딕"
            r.font.size = Pt(13.0)
            r.bold = True

        # Table 3: 계약 변동 이력 (Cell font 11pt)
        t3 = doc.tables[3]
        ...
        new_docx_path = os.path.join(u_path, new_docx_filename)

        try:
            doc.save(new_docx_path)
            print(f"    [CREATED PERFECT NOTE 11pt NO ENTERS] {new_docx_filename}")
            processed_count += 1
        except Exception as e:
            print(f"    [ERROR SAVING] {new_docx_filename}: {e}")

print(f"\n==========================================")
print(f"Finished processing 01_가산 to 04_광명! Total notes regenerated: {processed_count}")
print(f"==========================================")
