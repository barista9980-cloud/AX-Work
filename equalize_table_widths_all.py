...
# EXACT TOTAL TABLE WIDTH UNIFICATION (6.85 inches / 17.4 cm for all tables)
TOTAL_WIDTH_INCHES = 6.85

t0_cols = [1.40, 2.025, 1.40, 2.025] # Total = 6.85 in
t1_cols = [0.45, 2.95, 0.85, 1.75, 0.85]  # Total = 6.85 in
t2_cols = [1.40, 2.025, 1.40, 2.025] # Total = 6.85 in
t3_cols = [2.00, 4.85]               # Total = 6.85 in

processed_folders = 0
generated_notes = 0

print(f"Equalizing ALL table widths to EXACTLY {TOTAL_WIDTH_INCHES} inches across {FOXCONNECT_ROOT}...")
...
for root, dirs, files in os.walk(FOXCONNECT_ROOT):
    ...
    # Table 0: 계약 정보 (Total width 6.85 in)
    ...
    # Table 1: 수록 계약서 문서 목록 (Total width 6.85 in)
    ...
    # Re-enforce table column widths for all rows in Table 1
    set_table_width_and_columns(t1, t1_cols)
    ...
    # Table 2: 임대인 및 계좌 정보 (Total width 6.85 in)
    ...
    # Table 3: 계약 변동 이력 (Total width 6.85 in)
    ...
    # Save document with perfectly aligned table widths
    ...
    try:
        ...
        print(f"  [EXACT EQUAL TABLE WIDTHS APPLIED] {new_docx_filename}")
        generated_notes += 1
    except Exception as e:
...
print(f"\n==========================================")
print(f"Finished equalizing all table widths across all 29 property notes! Total folders: {processed_folders}")
