...
def create_contract_note_1510_style(master_info, docs_list, output_filepath):
    """
    Creates Contract Note following the 1510호 user-modified template style:
    - Table 2 (임대인 및 납부 계좌 정보):
      - Row 0: 임대인 | {임대인} | 임차인 | {임차인}
      - Row 1: 임대인 연락처 | {연락처} | 관리사무소 연락처 | {연락처}
      - Row 2: 입금 은행 | {은행} | 예금주 | {예금주}
      - Row 3: 계좌번호 (gridSpan=2) | {계좌번호} (gridSpan=2)
      - Row 4: 비고 (gridSpan=4, Header)
      - Row 5: 비고 데이터 (gridSpan=4, Data - Numbered 1. 2. 3. for Manager Ref)
    - Table 3 (계약 변동 이력 및 특이사항):
      - Row 0: 계약 변동 / 전대차 / 승계 이력 | {이력}
      - Row 1: 계약 연장 / 묵시적 갱신 이력 | {갱신}
      - Row 2: 중도해지 / 퇴거 예정 메모 | {퇴거}
      - Row 3: 기타 특약 및 참조사항 (gridSpan=2, Header)
      - Row 4: 기타 특약 및 참조사항 데이터 (gridSpan=2, Data - Numbered 1. 2. 3.)
    """
    doc = docx.Document()
    ...
    # Row 0: 임대인 | val | 임차인 | val
    # Row 1: 임대인 연락처 | val | 관리사무소 연락처 | val
    # Row 2: 입금 은행 | val | 예금주 | val
    # Row 3: 계좌번호 (gridSpan=2) | {계좌번호} (gridSpan=2)
    # Row 4: 비고 (gridSpan=4, Header)
    # Row 5: 비고 데이터 (gridSpan=4, Data - Numbered list)
    ...
    # Row 5: Merge ALL 4 cells for "비고" Data (Enlarged row with numbered items)
    t2.rows[5].cells[0].merge(t2.rows[5].cells[1]).merge(t2.rows[5].cells[2]).merge(t2.rows[5].cells[3])
    remarks_text = master_info.get("remarks_full", "1. 특이사항 없음\n2. 최초 임대차 계약 체결 및 전대차 승계 완료")
    ...
    # Row 4: Merge both cells for Data (Enlarged row with numbered special terms)
    t3.rows[4].cells[0].merge(t3.rows[4].cells[1])
    special_terms_text = master_info.get("special_terms_full", "1. 계약일 현재 등기부등본 확인 후 대상부동산의 권리 및 시설상태의 계약으로 한다.\n2. 임대차 만료 후 원상복구를 기본으로 한다.\n3. 부가세 및 관리비는 별도로 한다.")
    ...
    print("Successfully created 1510-style contract note:", output_filepath)


if __name__ == "__main__":
    # Create 402_403호 with the EXACT new 1510호 template style!
    target_402_403_dir = r"G:\내 드라이브\[FoxConnect]\[총무]업무\01_부동산_자산관리\01_임대차계약\01_가산_대륭포스트타워6차\402_403호"
    output_docx_path = os.path.join(target_402_403_dir, "부동산_계약관리노트_402_403호.docx")

    master_info_402_403 = {
        "building_name": "01_가산_대륭포스트타워6차",
        "unit_name": "402_403호",
        "usage": "가산 사무실 (콘텐츠본부, 전략본부)",
        "contract_type": "최초임대차 / 전대차",
        "initial_date": "2024-02-29",
        "period": "2024-02-29 ~ 2026-02-28",
        "payment_day": "매월 10일",
        "deposit": "46,000,000 원",
        "rent": "4,600,000 원",
        "area_m2": "218.7 ㎡",
        "area_pyung": "66.15 평",
        "lessor": "㈜엠씨에스솔루션 (이일우)",
        "lessee": "㈜폭스에듀 (전대인) → 정선혜 (전차인)",
        ...
        "remarks_full": (
            "1. 본 건은 402호, 403호 일괄 최초임대차계약(㈜엠씨에스솔루션-㈜폭스에듀) 후, 403호에 대해 정선혜와 전대차계약을 체결한 건임.\n"
            "2. 매월 임대료 납부일은 매월 10일이며, 신한은행 415-890001-13104 (예금주: ㈜엠씨에스솔루션) 계좌로 입금함.\n"
            "3. 403호 전대차계약 기간: 2025-07-01 ~ 2027-06-30 (보증금 10,000,000원 / 월세 1,000,000원)."
        ),
        "history_text": "2024-02-29 [최초임대차] ㈜엠씨에스솔루션 → ㈜폭스에듀\n2025-07-01 [전대차] ㈜폭스에듀 → 정선혜 (403호 전대차계약 체결)",
        "renewal_text": "현재 최초 계약 및 전대차 계약 유지 중",
        "termination_text": "특이사항 없음",
        "special_terms_full": (
            "1. 계약일 현재 등기부등본 확인 후 대상부동산의 권리 및 시설상태의 계약으로 한다.\n"
            "2. 임차인은 잔금시부터 관리비 및 임대료를 정상 부담하며, 임대차 만료 후 원상복구를 기본으로 한다.\n"
            "3. 차임 2기 이상 연체 시 임대차계약을 해지할 수 있으며, 명도 정산 절차를 따른다.\n"
            "4. 부가세 및 공과금, 관리비는 별도로 부담한다.\n"
            "5. 전대차계약 시 전대인은 임대인의 사전 동의를 득하고 진행하도록 한다."
        )
    }
    ...
    create_contract_note_1510_style(master_info_402_403, docs_list_402_403, output_docx_path)
    
    # Also update desktop reference copy
    desktop_402_403 = r"C:\Users\User\OneDrive\바탕 화면\부동산_계약관리노트_402_403호.docx"
    create_contract_note_1510_style(master_info_402_403, docs_list_402_403, desktop_402_403)
    print("Desktop 402_403호 master note updated successfully!")
