...
print(f"Applying Compact Page Fitting & Orphan Protection across {FOXCONNECT_ROOT}...")
...
for root, dirs, files in os.walk(FOXCONNECT_ROOT):
    ...
    # Set Page Margins for compact, elegant single-page fitting
    ...
    # Section 1 Heading: "1. 계약 정보" (11pt Bold, keep_with_next = True)
    ...
    # Table 0: 계약 정보 (5 rows, 4 cols, compact padding)
    ...
    # Section 2 Heading: "2. 수록 계약서 문서 목록 (총 N건)" (keep_with_next = True)
    ...
    # Section 3 Heading: "3. 임대인 및 납부 계좌 정보" (keep_with_next = True, NO EMPTY ENTERS BEFORE IT)
    ...
    # Section 4 Heading: "4. 계약 변동 이력 및 특이사항" (keep_with_next = True)
    ...
    for r_i, r_row in enumerate(t3.rows):
        ...
        for c_i, (val, is_h) in enumerate(row_vals):
            ...
            if is_h:
                ...
                align_d = WD_ALIGN_PARAGRAPH.CENTER if c_i == 0 else WD_ALIGN_PARAGRAPH.LEFT
                format_data_cell(cell, val, bold=False, font_size=10.0, align=align_d, bg_hex="FFFFFF")

    # Save compact 1-page fitted document
    ...
    try:
        ...
        print(f"  [PERFECT ONE-PAGE COMPACT FITTING COMPLETE] {new_docx_filename}")
        generated_notes += 1
    except Exception as e:
...
print(f"\n==========================================")
print(f"Finished applying perfect compact 1-page fitting! Total folders: {processed_folders}")
