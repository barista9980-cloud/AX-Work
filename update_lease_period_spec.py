...
print(f"Updating Lease Period OCR Rule Guidelines across {FOXCONNECT_ROOT}...")
...
for root, dirs, files in os.walk(FOXCONNECT_ROOT):
    ...
    # Lease period placeholder: Explain clearly or set YYYY-MM-DD ~ YYYY-MM-DD
    lease_period_str = f"{primary_item['contract_date']} ~ [종료일자 확인 필요]" if primary_item['contract_date'] else ""
    ...
    t0_data = [
        ...
        [("임대 기간", True), (lease_period_str, False), ("매월 납부일", True), ("", False)],
    ...
    try:
        ...
        print(f"  [LEASE PERIOD RULE APPLIED] {new_docx_filename}")
        generated_notes += 1
    except Exception as e:
...
print(f"\n==========================================")
print(f"Finished applying lease period OCR rule across all 29 property notes! Total folders: {processed_folders}")
