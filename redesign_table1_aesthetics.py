...
def format_cell(cell, text, bold=False, font_size=10.0, align=WD_ALIGN_PARAGRAPH.CENTER, color_rgb=None):
    cell.text = text
    p = cell.paragraphs[0]
    p.alignment = align
...
def parse_contract_filename(filename):
    ...
    if m:
        ...
        # Build a clean short display title (e.g. "402_403호 최초임대차계약서")
...
print(f"Applying Visual Redesign (Executive Dashboard Table Styling) to {FOXCONNECT_ROOT}...")
...
for root, dirs, files in os.walk(FOXCONNECT_ROOT):
    ...
    # Section 1 Heading: "1. 계약 정보" (11pt Bold)
    ...
    # Table 0: 계약 정보 (5 rows, 4 cols, 10pt, clean border #4B5563)
    t0 = doc.add_table(rows=5, cols=4)
    t0.alignment = WD_TABLE_ALIGNMENT.CENTER
    set_table_borders(t0, color="4B5563")
    ...
    for r_i, r_row in enumerate(t0.rows):
        ...
        for c_i, (val, is_h) in enumerate(row_vals):
            ...
            format_cell(cell, val, bold=is_h, font_size=10.0, align=WD_ALIGN_PARAGRAPH.CENTER)
            if is_h:
                set_cell_background(cell, "F1F5F9") # Soft Slate Header
    ...
    # Table 1: BEAUTIFULLY REDESIGNED EXEC DASHBOARD TABLE
    ...
    col_widths = [Inches(0.4), Inches(2.8), Inches(0.8), Inches(1.3), Inches(0.8)]
    ...
    for c_i, h in enumerate(t1_headers):
        ...
        format_cell(cell, h, bold=True, font_size=10.0, align=WD_ALIGN_PARAGRAPH.CENTER, color_rgb=RGBColor(30, 41, 59))
        set_cell_background(cell, "E2E8F0") # Elegant Soft Steel Gray Header
    ...
    for idx, c_it in enumerate(contract_items, 1):
        ...
        # Col 0: 순서
        format_cell(r_cells[0], seq_str, bold=True, font_size=10.0, align=WD_ALIGN_PARAGRAPH.CENTER, color_rgb=RGBColor(71, 85, 105))
        set_cell_background(r_cells[0], bg_color)
        r_cells[0].width = col_widths[0]

        # Col 1: 문서명 (CLEAN DUAL-LINE DESIGN: Bold Title + Gray Filename)
        cell1 = r_cells[1]
        cell1.width = col_widths[1]
        set_cell_background(cell1, bg_color)
        set_cell_margins(cell1, top=100, bottom=100, left=100, right=100)
        cell1.text = ""
        ...
        set_cell_background(r_cells[2], bg_color)
        r_cells[2].width = col_widths[2]

        # Col 3: 계약 당사자
        format_cell(r_cells[3], parties_str, bold=False, font_size=10.0, align=WD_ALIGN_PARAGRAPH.CENTER)
        set_cell_background(r_cells[3], bg_color)
        r_cells[3].width = col_widths[3]

        # Col 4: 계약일
        format_cell(r_cells[4], c_date, bold=False, font_size=10.0, align=WD_ALIGN_PARAGRAPH.CENTER)
        set_cell_background(r_cells[4], bg_color)
        r_cells[4].width = col_widths[4]
    ...
    # Table 2: 임대인 및 계좌 정보 (10pt, clean border #4B5563)
    t2 = doc.add_table(rows=4, cols=4)
    t2.alignment = WD_TABLE_ALIGNMENT.CENTER
    set_table_borders(t2, color="4B5563")
    ...
    # Table 3: 계약 변동 이력 (10pt, clean border #4B5563)
    t3 = doc.add_table(rows=4, cols=2)
    t3.alignment = WD_TABLE_ALIGNMENT.CENTER
    set_table_borders(t3, color="4B5563")
    ...
    try:
        ...
        print(f"  [BEAUTIFUL EXECUTIVE REDESIGN COMPLETE] {new_docx_filename}")
        generated_notes += 1
    except Exception as e:
...
print(f"\n==========================================")
print(f"Finished applying visual redesign to all property notes! Total folders: {processed_folders}")
