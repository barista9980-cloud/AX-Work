...
print(f"Applying 1510 Master Template Layout across ALL folders in {FOXCONNECT_ROOT}...")
...
for root, dirs, files in os.walk(FOXCONNECT_ROOT):
    ...
    remarks_text = f"1. 본 건은 {u_name} 최초임대차계약 건임.\n2. 최초 작성 시 원본 PDF 확인 후 관리자 검토를 진행해 주세요."
    ...
    try:
        ...
        print(f"  [1510 NEW MASTER TEMPLATE APPLIED] {new_docx_filename}")
        generated_notes += 1
    except Exception as e:
...
print(f"\n==========================================")
print(f"Finished applying 1510 Master Template Layout across all 29 property notes! Total folders: {processed_folders}")
