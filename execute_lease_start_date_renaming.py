...
print("Executing Lease Start Date Renaming for Real Estate PDFs in:", REAL_ESTATE_BASE)
...
def extract_lease_start_date(pdf_path, name_no_ext):
    try:
        ...
        for page in doc_pdf:
            ...
            if m_start:
                ...
                d_2d = f"{int(d):02d}"
                return f"{y_2d}{mon_2d}{d_2d}"
    except Exception:
        pass
    
    m_date = re.search(r"\((\d{6})\)", name_no_ext)
    return m_date.group(1) if m_date else "240101"
...
# STEP 1: Rename files with Lease Start Date
renamed_count = 0
...
for root, dirs, files in os.walk(REAL_ESTATE_BASE):
    ...
    for idx, f in enumerate(pdf_files, 1):
        ...
        lease_start_date = extract_lease_start_date(old_path, name_no_ext)

        pattern_a = r"^\d{2}_([^_]+)_(.*?)_\[(.*?)\]_\(\d{6}\)$"
        ma = re.match(pattern_a, name_no_ext)
        if ma:
            c_type, prop_info, parties = ma.groups()
            new_f = f"{seq_str}_{c_type}_{prop_info}_[{parties}]_({lease_start_date}).pdf"
        else:
            ...
            new_f = f"{seq_str}_{c_type}_{folder_name}_[{parties}]_({lease_start_date}).pdf"

        new_path = os.path.join(root, new_f)
        if old_path != new_path:
            try:
                os.rename(old_path, new_path)
                print(f"  [LEASE START DATE RENAMED] {f} -> {new_f}")
                renamed_count += 1
            except Exception as e:
                print(f"  [ERROR RENAMING] {f}: {e}")

print(f"\nTotal PDF Files Updated to Lease Start Date: {renamed_count}")
...
print(f"\n==========================================")
print(f"Finished! Renamed PDF files and regenerated {notes_count} Master Contract Notes with Lease Start Date!")
print(f"==========================================")
